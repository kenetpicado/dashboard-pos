<template>
    ...
    <nav class="flex items-center bg-white h-14 sticky top-0 shadow-lg">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4 container mx-auto">
        <div v-for="product in products.data" :key="product.id" class="flex max-w-md bg-white rounded-xl overflow-hidden shadow-lg">
            <img :src="product.image ?? '/not-found.jpg'" onerror="this.src='/not-found.jpg'" class="w-1/3 object-cover">
            <div class="w-4/3 p-3">
                <div class="flex flex-col justify-between h-full">
                    <span>
                        <div class="text-gray-800 font-semibold text-normal uppercase">
                            {{ cutString(product.name) }}
                        </div>
                        <p class="text-cyan-600 mt-1 text-xs">
                            {{ product.sku }}
                        </p>
                    </span>
                    <div class="flex item-center justify-between mt-3 text-gray-800">
                        <span class="text-lg">
                            C${{ getFirstPrice(product.inventory) }}
                        </span>
                        <span v-if="product.discount" class="badge-cyan">
